...
if not STABILITY_KEY:
    raise ValueError(
        "Не задан STABILITY_API_KEY. Проверь переменные окружения.")
...
hugs_file_ids = list(db.get("hugs_file_ids", []))


def save_photos_db():
    db["hugs_file_ids"] = hugs_file_ids
...
async def send_hug_with_photo(chat_id: int, text: str):
    if not hugs_file_ids:
        await bot.send_message(
            chat_id,
            text +
            "\n\n(пока нет картинок — отправь новую картинку в канал или сюда)",
            reply_markup=kb())
        return
    ...
    try:
        ...
        resp = client_groq.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                "Генерируй короткий, интересный факт без романтических обращений."
            }, {
                "role": "user",
        ...
        await call.message.answer(
            "Не получилось получить факт, попробуй чуть позже.",
            reply_markup=kb())
    ...
    try:
        ...
        resp = client_groq.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                ("Ты — внимательный и мягкий собеседник. "
...
async def cb_brawl(call: types.CallbackQuery):
    ...
    await call.answer()
    await call.message.answer(
        "Хочешь играть? Уже отправил уведомление Паше!!!", reply_markup=kb())
...
async def remind_text(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Когда прислать напоминание? Например: 'через 10 минут', 'завтра в 18:00', 'в понедельник в 12:30'"
    )
...
async def remind_time(message: types.Message, state: FSMContext):
    ...
    remind_dt = dateparser.parse(message.text,
                                 settings={'PREFER_DATES_FROM': 'future'})
    if not remind_dt:
        await message.answer(
            "Не смог понять время. Попробуй написать в формате 'через 10 минут' или 'завтра в 18:00'."
        )
        return
    ...
    await message.answer(
        f"Хорошо! Я напомню в {remind_dt.strftime('%H:%M %d.%m.%Y')} ✅")
    await state.clear()
...
# --------------------- Генерация картинки через Stability AI ---------------------
# ---- Добавлена функция translate_prompt, используется перед генерацией
async def translate_prompt(text: str) -> str:
    """
    Переводит русский промт на английский и оптимизирует его под генерацию изображения.
    Возвращает английский промт. В случае ошибки возвращает исходный текст.
    """
    try:
        client_groq = Groq(api_key=GROQ_API_KEY)
        resp = client_groq.chat.completions.create(
            ...
            messages=[{
                ...
                "system",
                "content":
                ("Ты — помощник по подготовке промтов для генерации изображений. "
...
async def draw_image(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        for resp in response:
            for artifact in resp.artifacts:
                if artifact.type == generation_pb2.ARTIFACT_IMAGE:
                    ...
                    # временный файл для FSInputFile
                    with tempfile.NamedTemporaryFile(suffix=".png",
                        ...
                        # отправляем с указанием оригинального и переведённого промта
                        caption = (
                            f"Вот что получилось по твоему описанию:\n\n«{prompt}»\n\n"
                            f"Использованный (англ.) промт: «{prompt_en}»")
        ...
        await message.answer(
            "Что-то пошло не так при генерации. Попробуй другой промт.",
            reply_markup=kb())
...
async def load_initial_photos():
    if not hugs_file_ids:
        try:
            with open('attached_assets/photos_1763399354936.json', 'r') as f:
                photos = json.load(f)
                hugs_file_ids.extend(photos)
                save_photos_db()
                logger.info(
                    f"Загружено {len(photos)} начальных фотографий в базу")
